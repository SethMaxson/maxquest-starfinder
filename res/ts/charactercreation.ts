		...
		for (let index = 0; index < item.subraces.length; index++) {
			...
			subraces.append(
				$(`<li class="subrace-option" name="${subItem.name}">
					<div class="header">
						<img src="/img/portraits/${subId}.png" class="token" onerror="imgError(this)" />
			...
			if (isChoose || name.length == 0) {
				...
				for (let index = 0; index < items.length; index++) {
					...
					let opt = $(`<li class="class-option" name="${item.name}">
						<div class="header">
							<img src="/img/classes/${item.name.toLowerCase()}.png" class="token" onerror="imgError(this)" />
							<div class="name">${item.name}</div>
						</div>
					</li>`);
					newBody.append(opt);
				...
				const description = `
					<div class="summary">
						...
						<div class="content fancy-details">
							<div><img src="/img/classes/${item.name.toLowerCase()}.png" class="token" onerror="imgError(this)" /></div>
							<div>${item.description}</div>
						</div>
					</div>
				`;
